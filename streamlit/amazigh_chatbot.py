...
def amazigh_stt(audio_path: str):
    ...
    return client.predict(
        audio_path, audio_path, "Tachelhit (shi)", api_name="/predict"
    )
...
def amazigh_resp(text: str):
    ...
    return tachelhit_response.choices[0].message.content


def amazigh_chatbot(req: str, req_type:str):
    text_amazigh_req = amazigh_stt(req) if req_type=='speech' else req
